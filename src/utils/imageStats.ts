...
export function calculateMissingImageTimestamps(
    ...
    const images = [...displayImages].sort(
        (a, b) => a.imageTimestamp - b.imageTimestamp
    )
    ...
    for (let i = 0; i < images.length - 1; i++) {
        ...
        if (gap > 1080) {
            ...
            while (expectedTime < nextTime - 180) {
                // Stop 3 minutes before the next image
                missingTimes.push({
