...
import {
    fetchInterestingImages,
    fetchWebcamImagesByDays,
    fetchWebcamImagesByTimestamp,
} from './api'
import {
    calculateMissingImageTimestamps,
    calculateImageStats,
} from './utils/imageStats'
...
function App() {
    ...
    const [displayedImage, setDisplayedImage] = useState<WebcamImage | null>(
        null
    )
    ...
    const {
        ...
        initialPageParam:
            initialData?.images[initialData.images.length - 1]?.imageTimestamp,
        queryFn: ({ pageParam }) => {
            if (!pageParam)
            ...
            return fetchWebcamImagesByTimestamp(
                0,
                pageParam - 1,
                inputLoadCount,
                'newestFirst'
    ...
    useQuery({
        ...
        queryFn: async () => {
            ...
            if (data.images.length > 0) {
                setToastMessage(
                    `${data.images.length} new image${data.images.length > 1 ? 's' : ''} available!`
                )
                setShowToast(true)
    ...
    const allImages = useMemo(() => {
        ...
        const uniqueImages = Array.from(
            new Map(images.map((img) => [img.imageId, img])).values()
        )
    ...
    const displayImages = useMemo(() => {
        ...
        console.log(
            '[Filter] Filtered to volcanic activity, count:',
            filtered.length
        )
        console.log(
            '[Filter] Sample of interesting codes:',
            allImages.slice(0, 10).map((img) => ({
                id: img.imageId,
    ...
    const missingImageTimestamps = useMemo(
        () => calculateMissingImageTimestamps(displayImages),
        [displayImages]
    )

    // Calculate expected vs actual images based on missing timestamps
    const imageStats = useMemo(
        () => calculateImageStats(displayImages, missingImageTimestamps),
    ...
    useEffect(() => {
        ...
        if (
            !displayedImage ||
            displayedImage.imageId === currentImage.imageId
        ) {
    ...
    useEffect(() => {
        ...
        console.log(
            '[Current Image] Display images count:',
        ...
        console.log(
            ...
            currentImage
                ? {
                      ...
                      timestamp: new Date(
                          currentImage.imageTimestamp * 1000
                      ).toLocaleString(),
                  }
                : 'No image'
        )
    ...
    const handleDateSubmit = async () => {
        ...
        try {
            ...
            if (data.images.length > 0) {
                ...
                setStartDate('')
                setToastMessage(
                    `Loaded ${data.images.length} images from ${new Date(startTimestamp * 1000).toLocaleDateString()}`
                )
                setShowToast(true)
                setTimeout(() => setShowToast(false), 5000)
            } else {
    ...
    const handleJumpToImage = async () => {
        ...
        try {
            ...
            const data = await fetchWebcamImagesByTimestamp(
                0,
                endTimestamp,
                Math.min(imagesToLoad, totalImages),
                'newestFirst'
            )
            ...
            if (data.images.length >= imageNumber) {
                ...
                setToastMessage(
                    `Could not load image ${imageNumber}. Only ${data.images.length} images available.`
                )
                setShowToast(true)
                setTimeout(() => setShowToast(false), 5000)
            }
                    ...
                    <>
                        {displayImages.length === 0 && showInterestingOnly ? (
                            <div className="card bg-base-200 shadow-xl p-8 text-center">
                                <h2 className="text-xl font-bold mb-4">
                                    No Volcanic Activity Images Found
                                </h2>
                                <p className="mb-4">
                                    There are no images marked with volcanic
                                    activity in the current dataset.
                                </p>
                                <div className="flex gap-4 justify-center">
                                    <button
                                        ...
                                        onClick={() => {
                                            console.log(
                                                '[No Images] Disabling volcanic activity filter'
                                            )
                            ...
                            <div className="space-y-4">
                                <div className="relative">
                                    ...
                                    <div className="absolute top-0 -right-36 flex flex-col gap-2 w-32">
                                        {displayedImage.interestingCode ===
                                            'V' && (
                                        ...
                                        >
                                            <div>
                                                ...
                                                {webcamInfo && (
                                                    <p className="text-sm text-base-content/50">
                                                        Total available:{' '}
                                                        {webcamInfo.imageTotal.toLocaleString()}
                                                    </p>
                                                )}
                                            </div>
                                        </button>
                                        ...
                                        <div className="form-control">
                                            <label className="label cursor-pointer gap-2">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        console.log(
                                                            '[Toggle] Volcanic activity filter changed to:',
                                                            e.target.checked
                                                        )
                                                        setShowInterestingOnly(
                                                            e.target.checked
                                                        )
                                                        setCurrentIndex(0)
                                                    }}
